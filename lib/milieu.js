...
class Milieu {
  ...
  explain() {
    ...
    this._resolveTemplatedVaules(envData);
    this._resolveTemplatedVaules(flagData);
    this._resolveTemplatedVaules(explanation);
    for (const config of configs) { this._resolveTemplatedVaules(config); }
  ...
  _compileConfig() {
    ...
    this._resolveTemplatedVaules(config);
  ...
  _resolveTemplatedVaules(config) {
    const templateObj = (ctx) => {
      for (const prop in ctx) {
        ...
        if (typeof ctx[prop] === 'string') {
          const typeTokenMatch = ctx[prop].match(/^\$[pbns]\|[^\|]+$/);
          if (typeTokenMatch) {
            ...
            switch (type) {
              case 'p': val = fs.readFileSync(key, 'utf8'); break;
              case 'b': val = !!envVal;                     break;
              case 'n': val = parseFloat(envVal || 0);      break;
              case 's':                                     break;
              default : val = '';                           break;
            }
          ...
          const strTokens = ctx[prop].match(/\$\{[^\}]+\}/g);
